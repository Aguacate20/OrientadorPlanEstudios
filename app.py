...
# Mantiene ocultamiento visual HIDE_VALUES y ahora sincroniza recomendaciones <---> checkboxes/options
...
with st.spinner("Cargando datos iniciales..."):
    ...
    # build_curriculum_graph ahora recibe program como primer argumento (ver cambios previos)
...
def update_plan():
    ...
    # ---- SIN CAMBIAR LÓGICA: sincronizar RECOMENDACIONES con las opciones de UI ----
    # Es decir: si el algoritmo recomendó is_half_time / extra_credits / intersemestral
    # para un semestre, escribirlo en st.session_state.semester_options para que
    # la casilla/slider/select quede activada en la interfaz.
    for sem_entry in plan:
        ...
        # SOBREESCRIBIMOS con la recomendación (el usuario puede cambiarlo luego y pulsar "Recalcular")
        ...
        # intersemestral puede ser None o nombre
...
if st.session_state.plan:
    ...
    # Iteramos *sobre la plan calculado* para mostrar cada pestaña y además usar
    # la lista de materias recomendadas en ese semestre para calcular intersemestrales
    for i, (tab, semester_plan) in enumerate(zip(tabs, st.session_state.plan)):
        with tab:
            ...
            # ---- Intersemestral: ahora calculamos opciones *teniendo en cuenta* las materias
            # recomendadas para este mismo semestre (si el usuario acepta la recomendación) ----
            # Construimos un conjunto temporal: aprobadas actuales + materias recomendadas para este semestre
            temp_approved_for_inter = set(st.session_state.approved_subjects)
            rec_subjects_for_this_sem = semester_plan.get("subjects", [])
            # No agregamos intersemestrales del plan aquí; sólo las materias recomendadas en el semestre
            ...
            # Si el plan ya recomendó un intersemestral, incluirlo en el listado (si no estaba en options)
